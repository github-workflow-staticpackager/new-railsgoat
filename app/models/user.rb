...
class User < ApplicationRecord
  ...
  def self.authenticate(email, password)
    ...
    sha1_digest = Digest::SHA1.hexdigest(password) # CWEID 327
    rmd160_digest = Digest::RMD160.hexdigest(password) # CWEID 327

    raise "#{email} doesn't exist!" if !(user)
    if user.password == Digest::MD5.hexdigest(password) # CWEID 327
      auth = user
    else
  ...
  def hash_password
    if will_save_change_to_password?
      self.password = Digest::MD5.hexdigest(self.password) # CWEID 327
    end
  end
